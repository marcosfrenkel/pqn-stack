...
async def reset_coordination_state() -> None:
    """Reset the coordination state of the node."""
    state.leading = False
    state.followers_address = None
    ...
    if state.client_listening_for_follower_requests and not state.following:
        ...
        logger.info(
            ...
            (
                ...
                if state.client_listening_for_follower_requests
...
async def follow_requested_alert(websocket: WebSocket) -> None:
